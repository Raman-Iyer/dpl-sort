...
const Teams = () => {
  const teamData: TeamType[] = JSON.parse(localStorage.getItem("teams") || "");
        ...
        return (
          <div className="w-full flex items-center border-2 gap-8 p-5">
            ...
            <div className="flex flex-col gap-2 h-full">
              ...
              <h2 className="text-3xl font-bold">Captain: {captain.name}</h2>
              <h3 className="text-2xl font-bold">
                Flag Bearer: {viceCaptain.name}
              </h3>
              <p className="font-bold text-xl">Team: </p>
              <div>
                {team.map((playerName) => {
                  return <p>- {playerName.name}</p>;
                })}
              </div>
            </div>
          </div>
